...
from ..indexer.pg_common import POSTGRES_DB, POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_HOST
...
class BaseDB:
    ...
    def _fetchone(self, values, keys) -> str:
        ...
        self.debug(f'SELECT {",".join(values)} FROM {self._table_name} WHERE {where_cond}')
