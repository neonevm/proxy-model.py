...
class Test_Neon_Faucet(unittest.TestCase):
    ...
    def setUpClass(cls):
        print('\n\n' + issue)
        cls.proxy = Proxy()
        cls.admin = cls.proxy.create_account(issue + '/admin')
        cls.user = cls.proxy.create_account(issue + '/user')
    ...
    def test_neon_faucet_02_neon_in_galans(self):
        ...
        balance_before = self.proxy.conn.get_balance(self.user.address)
        print('NEO balance before:', balance_before)
        data = '{"wallet": "' + self.user.address + '", "amount": 99999}'
        r = requests.post(url, data=data)
        if not r.ok:
            print('Response:', r.status_code)
        self.assertTrue(r.ok)
        # Check
        balance_after = self.proxy.conn.get_balance(self.user.address)
    ...
    def test_neon_faucet_03_neon(self):
        ...
        balance_before = self.proxy.conn.get_balance(self.user.address)
        print('NEO balance before:', balance_before)
        data = '{"wallet": "' + self.user.address + '", "amount": 1}'
        r = requests.post(url, data=data)
        if not r.ok:
            print('Response:', r.status_code)
        self.assertTrue(r.ok)
    ...
    def test_neon_faucet_06_erc20_single(self):
        ...
        before = self.get_token_balance(token, self.user.address)
        print('token A balance before:', before)
        data = '{"wallet": "' + self.user.address + '", "token_addr": "' + token + '", "amount": 1}'
        print('data:', data)
        r = requests.post(url, data=data)
        if not r.ok:
            print('Response:', r.status_code)
        self.assertTrue(r.ok)
        after = self.get_token_balance(token, self.user.address)
    ...
    def test_neon_faucet_05_erc20_all(self):
        print()
        url = '{}/request_erc20'.format(os.environ['FAUCET_URL'])
        a_before = self.get_token_balance(self.token_a, self.user.address)
        b_before = self.get_token_balance(self.token_b, self.user.address)
        ...
        if not r.ok:
            print('Response:', r.status_code)
        self.assertTrue(r.ok)
        a_after = self.get_token_balance(self.token_a, self.user.address)
        b_after = self.get_token_balance(self.token_b, self.user.address)
    ...
    def get_token_balance(self, token_address, address):
        erc20 = self.proxy.conn.contract(address=token_address, abi=self.contract['abi'])
