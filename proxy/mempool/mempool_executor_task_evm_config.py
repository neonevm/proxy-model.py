...
from ..neon_core_api.neon_layouts import BPFLoader2ProgramInfo, BPFLoader2ExecutableInfo, EVMConfigData
...
class MPExecutorEVMConfigTask(MPExecutorBaseTask):
    ...
    def _get_evm_config(self, src_cfg_data: EVMConfigData) -> EVMConfigData:
        ...
        if program_info.executable_addr == SolPubKey.default():
            return EVMConfigData.init_empty()
        ...
        for key, value in evm_cfg_data.evm_param_dict.items():
            if key not in src_cfg_data.evm_param_dict:
