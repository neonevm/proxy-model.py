...
ssh -i ${SSH_KEY} ubuntu@${REMOTE_HOST} 'sudo docker logs solana | pbzip2 > /tmp/solana.log.bz2'
...
do
   echo "$service"
   ssh -i ${SSH_KEY} ubuntu@${REMOTE_HOST} "sudo docker logs $service | pbzip2 > /tmp/$service.log.bz2"
